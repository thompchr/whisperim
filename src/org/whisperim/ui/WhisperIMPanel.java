...
ChangeListener {
	...
	// This needs to be changed to support using the icon associated with the
	// plugin
	...
	private HTMLDocument recText = (HTMLDocument) editorKit
	.createDefaultDocument();
	private HTMLDocument sendText = (HTMLDocument) editorKit
	.createDefaultDocument();
			...
			public void hyperlinkUpdate(HyperlinkEvent arg0) {
				...
				try {
					if (arg0.getEventType() == HyperlinkEvent.EventType.ENTERED) {
						...
						HyperlinkWindow w = new HyperlinkWindow(
						"http://google.com");
				...
				try {
					if (arg0.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
						...
						if (arg0.getURL().toString().contains("mailto://")) {
							...
							command
							.add("C:\\Program Files\\Skype\\Phone\\Skype.exe");
							command.add("/callto:+1"
									+ arg0.getURL().toString().replace(
											"mailto://", ""));
							 ...
							 * java.awt.Desktop.getDesktop(); try {
							 * 
							 * java.net.URI uri = new
							 * java.net.URI(arg0.getURL().toString());
							 * desktop.browse( uri ); } catch ( Exception e ) {
							 * 
							 * System.err.println( e.getMessage() ); }
							 */

							BareBonesBrowserLaunch.openURL(arg0.getURL()
									.toString());
						}
					}
				} catch (Exception e) {
...
private JComponent initComponents() {
	...
	buddyName_ = new JLabel(buddy_.getHandle(), serviceIcon_,
			SwingConstants.LEFT);
	...
	messageArea_.setTransferHandler(window_.new TabTransferHandler(
			window_.new PanelManager(this)));
	messageArea_.addKeyListener(new KeyAdapter() {
	...
	.setVerticalGroup(layout.createParallelGroup(
			...
			.add(
					...
					.addContainerGap().add(buddyName_,
...
public void actionPerformed(ActionEvent e) {
	...
	if (actionCommand.equals(sendBtn_.getActionCommand())) {
		...
		try {
			Message keyMsg = new Message(new Buddy(myHandle_, myHandle_,
					buddy_.getProtocolID()), new Buddy(theirHandle_,
							myHandle_, buddy_.getProtocolID()),
							"<whisperim keyspec=" + Encryptor.getMyPublicKey()
							+ "-- />", buddy_.getProtocolID(), Calendar
							.getInstance().getTime());
		...
		if (whiteboardBtn_.isEnabled()) {
			try {
				...
				updateChatArea(
						"An error has occurred starting the whiteboard.",
				"");
			}
...
public void autoScroll() {
	// this code implements autoscroll if the scroll bar is at the end of
	// the box
	...
	// boolean autoScroll = ((bar.getValue() + bar.getVisibleAmount()) ==
	// bar.getMaximum());
...
public void updateChatArea(String newText, String info) {
	...
	if ((tempRecMessage.indexOf("<HTML>") != -1)
			|| (newText.contains("<font>"))) {

		tempRecMessage = info
		+ tempRecMessage.substring(
				tempRecMessage.indexOf("<BODY>") + 6,
				tempRecMessage.indexOf("</BODY>"));
		String finalText = oldText
		.substring(0, oldText.indexOf("</p>") - 1)
		+ "<br>"
		+ tempRecMessage
		+ oldText.substring(oldText.indexOf("</p>"), oldText
				.length());
		talkArea_.setText(finalText);
	} else {
		newText = "<FONT FACE=\"Arial\" SIZE=4 COLOR=#000000>" + info
		+ newText + "</FONT>";
		String finalText = oldText
		.substring(0, oldText.indexOf("</p>") - 1)
		+ "<br>"
...
public void receiveMsg(Message message) throws BadLocationException {
	...
	String info = "<FONT COLOR=#FDD017><b>" + "("
	+ d.format(message.getTimeSent()) + ") " + message.getFrom()
	+ ": " + "</b></FONT>";
	...
	if (!doEncryption_ || !message.getMessage().contains("<key>")) {
		...
		String updatedInfo = info + "<FONT COLOR=#FF0000><b>"
		+ " (Encrypted) " + "</b></FONT>";
...
public boolean isWhiteboardMsg(String msg) {
	if (msg.contains("<whiteboard>") && msg.contains("</whiteboard>"))// contains
		// whiteboard
		// tags
	{
...
public void sendMsg() {
	...
	if (messageArea_.getText().trim().equals("")) {
		...
		if (doEncryption_) {
			...
			updateChatArea("<FONT COLOR=#F87217><b>" + "(" + df1.format(d)
					+ ") " + myHandle_ + ": "
					+ "</FONT><FONT COLOR=#FF0000>" + "(Encrypted) "
					+ "</b></FONT>" + messageText + "\n", "");
			messageText = encrypt
			.generateCipherText(messageArea_.getText());
			...
			updateChatArea("<FONT COLOR=#F87217><b>" + "(" + df1.format(d)
					+ ") " + myHandle_ + ": " + "</b></FONT>" + messageText
					+ "\n", "");
			messageText = messageArea_.getText();
			messageArea_.repaint();
		}

		Message message = new Message(new Buddy(myHandle_, myHandle_,
				buddy_.getProtocolID()), new Buddy(theirHandle_, myHandle_,
						buddy_.getProtocolID()), messageText, buddy_
						.getProtocolID(), Calendar.getInstance().getTime());
...
public void flash(boolean focused) {
	...
	if (head_ == null) {
		...
		System.out
		.println("Please contact your favorite WhisperIM dev with error code NOT1337");
	} else {
		if (focused) {
...
public JPanel getTabHead() {
	...
	if (head_ == null) {
		...
		close_.setPreferredSize(new Dimension(closeIcon_.getIconHeight(),
				closeIcon_.getIconWidth()));
