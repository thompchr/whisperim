...
public class Chat extends DiscoveryClientServer implements DiscoveryListener,
		PipeMsgListener, Runnable, PipePresence.Listener,
	...
	private EndpointReceiveQueue queue_ = new EndpointReceiveQueue(
			MAX_MESSAGES_);
	...
	// Chat constructor.
	protected Chat(PeerGroupManager manager, String attr, String value) {
	...
	private PipePresence getPipePresence(PeerGroup g,
		...
		PipePresence tmpPresence = (PipePresence) pipePTable_.get(g
				.getPeerGroupID().toString());
		if (tmpPresence == null) {
			// No PipePresence has been created yet.
			tmpPresence = new PipePresence(g, localPipeAdv);
			pipePTable_.put(manager_.getSelectedPeerGroup().getPeerGroupID()
	...
	public synchronized void groupChanged(PeerGroup group) {
		...
		try {
			...
			while (localAds != null && localAds.hasMoreElements()) {
				PipeAdvertisement adv = (PipeAdvertisement) localAds
						.nextElement();
			...
			processPeerStructureChanged(new AdvertisementEvent(this,
					AdvertisementEvent.GRP_CHANGED, result.elements()));
		} catch (IOException e) {
	...
	public void startChat(PipeAdvertisement toWhom) throws Exception {
		...
		if (toWhom != null) {
			...
			currentUser_ = getPipePresence(group, myPipeAdvt_).getOnlineBuddy(
					getName(toWhom));
	...
	public static void pingBuddy(BuddyListBuilder buddy) {
		 ...
		 * msg.setString (SRCPEERADV_, myPeerAdvString_); msg.setString
		 * (GROUPID_, buddy.getGroup().getPeerGroupID().toString());
		 * msg.setString (COMMAND_, PING_); msg.setString (SENDERNAME_,
		 * myName_);
		 */

		msg.setMessageProperty(SRCPIPEADV_, advToString(buddy.getReplyPipe()));
		msg.setMessageProperty(SRCPEERADV_, myPeerAdvString_);
		msg.setMessageProperty(GROUPID_, buddy.getGroup().getPeerGroupID()
				.toString());
	...
	public void probeUser(PipeAdvertisement toWhom) {
		PeerGroup group = manager_.getSelectedPeerGroup();
		getPipePresence(group, myPipeAdvt_).addOnlineBuddy(getName(toWhom),
				toWhom);
	}
	...
	private Enumeration findLocalUsers() {
		...
		try {
			return discovery_.getLocalAdvertisements(DiscoveryService.ADV,
					PipeAdvertisement.NameTag, CHATNAMETAG_ + ".*");
	...
	public void sendMessageToPeers(String message) {
		...
		if (currentUser_ != null) {
			...
			try {
				...
				// push my GroupName
				// msg.setString(
				msg.setMessageProperty(SENDERGROUPNAME_, manager_
						.getSelectedPeerGroup().getPeerGroupName());
	...
	public synchronized void runChatServer() {
		...
		if (myPipeAdvt_ == null) {
			...
			myPipeAdvt_ = (PipeAdvertisement) AdvertisementFactory
					.newAdvertisement(PipeAdvertisement.getAdvertisementType());

			myPipeAdvt_.setPipeID(IDFactory.newPipeID((PeerGroupID) group
					.getPeerGroupID()));
	...
	public synchronized void publishAdvertisement(Advertisement adv,
		...
		discovery.publish(adv, discovery.ADV, discovery.DEFAULT_LIFETIME);
	...
	public void searchAdvertisements() throws IOException {
		discovery_.getRemoteAdvertisements(null, DiscoveryService.ADV,
				PipeAdvertisement.NameTag, CHATNAMETAG_ + ".*",
				DiscoveryClientServer.THRESHOLD_, null);

		// Get local values.
		Enumeration localAds = discovery_.getLocalAdvertisements(
				DiscoveryService.ADV, PipeAdvertisement.NameTag, CHATNAMETAG_
						+ ".*");
	...
	public void presenceEvent(PipePresence.PresenceEvent event) {
		...
		if (event.getName().equals(myName_) && !event.getStatus()) {
			// Ignore the event
			return;
		}
