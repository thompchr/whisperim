...
public class WhisperBot  implements AccEvents {
	...
	public WhisperBot() throws AccException
	{
		...
		prefs.setValue("aimcc.im.standard.permissions.nonBuddies", AccPermissions.RejectAll);
	...
	public void OnImReceived(AccSession session, AccImSession imSession, AccParticipant participant, AccIm im) {
		try { 
			...
			if(msg.equals("$stats"))
	        {
				...
				// Separate out $encryptedSendFile from file name to be transfered and format
				int i = msg.length();
				...
				// Send File
				AccFileXferSession fileXferSession = session.getFileXferManager().send(participant.getName(), encryptedFileName, "Transfering file...", null);
	        }else if (msg.startsWith("$sendFile")) {
				
				// Separate out $sendFile from file name to be transfered and format
				int i = msg.length();
				...
				// Send File
				AccFileXferSession fileXferSession = session.getFileXferManager().send(participant.getName(), fileName, "Transfering file...", null);
			}else if(msg.equals("$directory"))
			{
				...
				session.createIm(filesArray.toString(), null);
				
			}else if(msg.startsWith("help"))
	        {
	            richText = "$stats<br>$directory<br>$sendFile<br>$help<br>";
	        }
