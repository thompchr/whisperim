...
import org.whisperim.ui.WhisperIMPanel;
...
public class DownloadManager extends JFrame implements Observer {
	...
	public DownloadManager() {
		...
		table_.setRowHeight((int) renderer.getPreferredSize().getHeight());

		// Set up downloads panel.
		JPanel downloadsPanel = new JPanel();
		downloadsPanel.setBorder(BorderFactory.createTitledBorder("Downloads"));
		downloadsPanel.setLayout(new BorderLayout());
		downloadsPanel.add(new JScrollPane(table_), BorderLayout.CENTER);
	...
	private void encryptFile() {
		Object userInput = JOptionPane.showInputDialog(null,
				"Enter file path.", "Enter your file",
				JOptionPane.QUESTION_MESSAGE);
		if (userInput != null) {
			if (userInput.toString().length() > 150) {
				FileEncryptor fe = new FileEncryptor(Encryptor
						.getPublicKeyForBuddy(getName()), null);
				fe.generateCipherFile((File) userInput, "Encrypted File");
			}
		}
	}
	...
	private void decryptFile() {
		...
		if (userInput != null) {
			if (userInput.toString().length() > 150) {
				FileEncryptor fe = new FileEncryptor(null, null);
				fe.decryptFile((File) userInput);
			}
		}
	...
	private void tableSelectionChanged() {
		 ...
		 */
		if (!clearing_) {
			selectedDownload_ = tableModel_
					.getDownload(table_.getSelectedRow());
