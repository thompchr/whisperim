...
public class PeerGroupManager implements AdvertisementDiscoveryListener {
	...
	public Enumeration getGroups() {
		...
		while (ads != null && ads.hasMoreElements()) {
			PeerGroupAdvertisement adv = (PeerGroupAdvertisement) ads
					.nextElement();
	...
	public Enumeration getGroupsLeft() {
		...
		while (all.hasMoreElements()) {
			PeerGroupAdvertisement adv = (PeerGroupAdvertisement) all
					.nextElement();
	...
	public PeerGroup getSelectedPeerGroup() {
		...
		if (!myGroups_.contains(currentGroup_))
			throw new RuntimeException(
					"Selected group is not one of the joined groups!");
	...
	public void searchGroups(String groupName) {
		try {
			...
			buffer
					.append("Could not retrieve the local peer group advertisements");
	...
	public void searchPeers(String peerName) {
		try {
			...
			buffer.append("Could not retrieve the local peer advertisements");
			if (iox.getMessage() != null) {
				buffer.append(EOL).append(iox.getMessage());
			}
		}
	...
	public void selectedGroupChanged(PeerGroupAdvertisement adv) {
		...
		prefs.setProperty(getClass().getName() + SELECTED_PEER_GROUP, adv
	...
	public void joinGroup(PeerGroup pg, PeerGroupAdvertisement pgAdv,
		...
		try {
			if (getIndexOfAdv(myGroupAdvs_, pgAdv) != -1) {
				throw new RuntimeException(pgAdv.getName()
			...
			processGroupStructureChanged(new AdvertisementEvent(this,
	...
	public void createPeerGroup(String name, boolean isRendezVous,
			String password) {

		try {
			// Instantiate group from the impl adv.
			ModuleImplAdvertisement newGroupImpl = rootPeerGroup_
			...
			if (null != password) {
				...
				for (Enumeration eachModule = (Enumeration) services.keySet(); eachModule
						.hasMoreElements();) {
					...
					if (aModuleClassID.equals(PeerGroup.membershipClassID)) {
						ModuleImplAdvertisement aModuleAdv = (ModuleImplAdvertisement) services
								.get(aModuleClassID);
						services.remove(aModuleClassID);

						ModuleImplAdvertisement implAdv = (ModuleImplAdvertisement) AdvertisementFactory
						...
						implAdv
								.setCode(PasswdMembershipService.class
										.getName());
						...
						newGroupImpl.setParam((Element) params
				...
				if (replacedMembership) {
					...
					if (!newGroupImpl.getModuleSpecID().equals(
						...
						newGroupImpl.setModuleSpecID(IDFactory
								.newModuleSpecID(newGroupImpl.getModuleSpecID()
										.getBaseClass()));
					} else {
						ID passwdGrpModSpecID = ID.nullID;

						try {
							passwdGrpModSpecID = IDFactory
									.fromURL(new URL(
											"urn",
											"",
											"jxta:uuid-"
			...
			rootPeerGroup_.getDiscoveryService().publish(newGroupImpl,
					DiscoveryService.ADV, PeerGroup.DEFAULT_LIFETIME);
			...
			if (null != password) {
				StructuredTextDocument params = (StructuredTextDocument) StructuredDocumentFactory
	...
	public synchronized void leaveGroup(PeerGroupAdvertisement group)
		...
		selectedGroupChanged((PeerGroupAdvertisement) myGroupAdvs_.elementAt(0));
	...
	// Blocks until a RendezVousService server is found and connection is
	// started.
	...
	private int getIndexOfAdv(Vector vector, PeerGroupAdvertisement pgAdv) {
		...
		for (int i = 0; i < vector.size(); i++) {
			PeerGroupAdvertisement targetAdv = (PeerGroupAdvertisement) vector
					.elementAt(i);
	...
	private void processPeerStructureChanged(AdvertisementEvent e) {
		for (int i = 0; i < structureListeners_.size(); i++) {
			GroupStructureListener l = (GroupStructureListener) structureListeners_
					.elementAt(i);
			l.peerDataChanged(e);
		}
	}
	...
	private void processGroupStructureChanged(
			AdvertisementEvent advertismentEvent) {
		for (int i = 0; i < structureListeners_.size(); i++) {
	...
	private void updateJoinedGroups() {
		...
		while (groupAdv.hasMoreElements()) {
			id = ((PeerGroupAdvertisement) groupAdv.nextElement())
					.getPeerGroupID().toString();
			...
			if (type == DiscoveryService.PEER) {
				...
				if (ad1 != null && ad2 != null) {
					...
					try {
						...
						result = p1.getPeerGroupID()
								.equals(p2.getPeerGroupID());
					} catch (ClassCastException iox) {
						result = false;
					}
				}
